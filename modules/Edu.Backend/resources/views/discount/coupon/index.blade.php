...
<div class="tabs-container">
    ...
    <ul class="nav nav-tabs">
        <li class="{{ Active::query('status','') }}"><a href="{{route('edu.discount.index')}}" no-pjax> 所有优惠券
                <span class="badge"></span></a></li>
        <li class="{{ Active::query('status','nstart') }}"><a
                    href="{{route('edu.discount.index',['status'=>'nstart'])}}" no-pjax> 未开始
                <span class="badge"></span></a></li>
        <li class="{{ Active::query('status','ing') }}"><a
                    href="{{route('edu.discount.index',['status'=>'ing'])}}" no-pjax> 进行中
                <span class="badge"></span></a></li>
        <li class="{{ Active::query('status','end') }}"><a
                    href="{{route('edu.discount.index',['status'=>'end'])}}" no-pjax> 已结束
                <span class="badge"></span></a></li>
    </ul>
    ...
    <div class="tab-content">
        <div id="tab-1" class="tab-pane active">
            ...
            {!! Form::open( [ 'route' => ['edu.discount.index'], 'method' => 'get', 'id' => 'discount-form','class'=>'form-horizontal'] ) !!}
            <div class="panel-body">
                <div class="row">
                    <div class="col-md-6">
                        <a class="btn btn-primary " href="{{ route('edu.discount.create') }}" no-pjax>添加优惠券</a>
                    </div>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        <a
                                                ...
                                                href="{{route('edu.discount.edit',$item->id)}}" no-pjax>
                                            <i data-toggle="tooltip" data-placement="top"
                                               class="fa fa-pencil-square-o"
                                               title="编辑"></i></a>

                                        <a href="{{route('edu.discount.showCoupons',['id'=>$item->id])}}"
                                           class="btn btn-xs btn-primary" no-pjax>
                                            <i class="fa fa-asterisk" data-toggle="tooltip" data-placement="top"
                                               title="查看领取记录"></i></a>


                                        <a href="{{route('edu.discount.useRecord',['id'=>$item->id])}}"
                                           class="btn btn-xs btn-primary" no-pjax>
